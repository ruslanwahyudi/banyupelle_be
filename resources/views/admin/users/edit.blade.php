				...
				<div class="card-body">
					...
					<form action="{{ route('admin.users.update', ['user' => $user->id]) }}" method="post">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" value="{{ $user->name }}" id="name" name="name" aria-describedby="emailHelp" placeholder="Masukkan nama user" required>
							<!-- <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> -->
						</div>

						<div class="form-group">
							<label for="email">Email</label>
							<input type="text" class="form-control" value="{{ $user->email }}" id="email" name="email" aria-describedby="emailHelp" placeholder="Masukkan email" required>
						</div>	
